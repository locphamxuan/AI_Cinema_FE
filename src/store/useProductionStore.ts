/**
 * AI Cinema - Production & Publishing Store (Maker-Checker Flow)
 */
...
interface ProductionStoreState {
  ...
  activeRole: ProductionRole; // 'reviewer' | 'creator'
...
export const useProductionStore = create<ProductionStoreState>((set, get) => ({
  ...
  activeRole: 'reviewer', // Mặc định role Reviewer để thẩm định
  ...
  generateSceneVideo: async (projectId, episodeId, sceneId) => {
    ...
    if (currentUsed + needed > quota) {
      return {
        success: false,
        error: `Vượt quá giới hạn hạn ngạch AI Token! (Hiện tại: ${currentUsed}/${quota} Tokens, Cần thêm: ${needed} Tokens).`,
      };
    }

    // Set rendering state
    get().updateScene(projectId, episodeId, sceneId, { status: 'rendering', progress: 15 });

    // Simulate progress 0% -> 100%
    await new Promise((r) => setTimeout(r, 400));
    get().updateScene(projectId, episodeId, sceneId, { progress: 50 });

    await new Promise((r) => setTimeout(r, 400));
    get().updateScene(projectId, episodeId, sceneId, { progress: 85 });
  ...
  submitEpisodeForReview: (projectId, episodeId) => {
    ...
    if (uncompleted.length > 0) {
      return {
        success: false,
        error: `Còn ${uncompleted.length} phân cảnh chưa hoàn tất render AI. Vui lòng hoàn thành trước khi nộp.`,
      };
    }
    ...
    const feedback: FeedbackItem = {
      ...
      role: 'creator',
      type: 'approval',
      content: `Đã hoàn thiện tất cả ${ep.scenes.length} phân cảnh và nộp bản dựng hoàn chỉnh lên Reviewer.`,
      createdAt: new Date().toISOString(),
    };
  ...
  respondToTokenExtension: (projectId, requestId, approve, notes) => {
    set((state) => ({
      projects: state.projects.map((proj) => {
        ...
        // If approved, allocate quota directly to the episode
